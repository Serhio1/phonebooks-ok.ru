...
if ( ! function_exists('retro_comments') ) {
	...
	function retro_comments( $comment, $args, $depth ) {
		...
		<span>
			...
			// <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __('Ответить', 'haku' ), 'depth' => $depth, 'max_depth' => $args['max_depth']) ) ); ?>
			<?php edit_comment_link( __('(Редактировать)', 'haku') ); ?>
		</span> 
		
		<?php if ( $awaiting ) : ?><em><?php _e( 'Ваш комментарий проходит проверку.', 'haku' ); ?></em><?php else: ?><?php comment_text(); ?><?php endif; ?>
